...
const getAllPropertyCategory = async (req, res, next) => {
	// console.log('getAllPropertyCategory', req.query);
	...
	try {
		...
		let aggregateData = await PropertyCategory.aggregate([
			{ $match: conditionObj },
			{ $sort: { created_at: -1 } },
			{
				$facet: {
		...
		res.status(200).json({
			status: true,
			message: 'all property category listing',
			data,
			meta_data,
		});
...
module.exports = getAllPropertyCategory;