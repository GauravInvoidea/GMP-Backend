...
const getAllPropertyAmenities = async (req, res, next) => {
	// console.log('getAllPropertyAmenities', req.query);
	...
	try {
		...
		let aggregateData = await PropertyAmenity.aggregate([
			{ $match: conditionObj },
			{ $sort: { created_at: -1 } },
			{
				$facet: {
		...
		res.status(200).json({
			status: true,
			message: 'all property amenities listing',
			data,
			meta_data,
		});
...
module.exports = getAllPropertyAmenities;