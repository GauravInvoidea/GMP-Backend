...
exports.addorUpdateGeneralData = async (req, res) => {
    try {
        ...
        if (isAdmin === true || isAdmin === 'true') {
            ...
            const isDocumentPresent = await SystemSetting.find();

            // console.log(updateFields)

            if (isDocumentPresent.length === 0) {
                // No document exists, create a new one
                const newData = new SystemSetting(updateFields);
                ...
                const updatedData = await SystemSetting.findOneAndUpdate({}, updateFields, { new: true });
